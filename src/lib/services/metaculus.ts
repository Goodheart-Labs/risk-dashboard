...
export async function fetchMetaculusData(questionId: number): Promise<ChartDataPoint[]> {
  try {
    // Try API first
...
function transformMetaculusData(data: MetaculusResponse): ChartDataPoint[] {
  ...
  const result = data.question.aggregations.recency_weighted.history.map(point => ({
